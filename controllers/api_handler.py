...
def call_ai_api(user_message_text, is_toggle_on, gemini_chat, openai_client, image_files=None, document_files=None, parent_widget=None): # Thêm parameters image_files và document_files
    """
    Gọi AI API (OpenAI hoặc Gemini) dựa trên toggle state, hỗ trợ file ảnh và tài liệu cho Gemini (phiên bản đơn giản - KHÔNG dùng Part/Content).
    ...
    Returns:
        tuple: (bot_reply_text, ai_sender) - Phản hồi từ AI và sender.
    ...
    try:
        ...
        if is_toggle_on: # Toggle ON: OpenAI/ChatGPT
            print("Gọi OpenAI/ChatGPT API từ api_handler.py (chưa hỗ trợ file)") # Cập nhật log message
            ...
            print("Gọi Gemini API từ api_handler.py (có hỗ trợ file - phiên bản đơn giản)") # Cập nhật log message
            ...
            gemini_response = gemini_chat.send_message(content=contents) # Gọi generate_content() với danh sách contents
            bot_reply_text = gemini_response.text
            ai_sender = "system"
            # print(f"Gemini history: {gemini_chat.history}")
    ...
    return bot_reply_text, ai_sender